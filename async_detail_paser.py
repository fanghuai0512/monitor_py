...
import get_response as get_response
...
class DetailParse:
    ...
    def __init__(self, url: str, response=None, task_data=None,
        ...
        # asin
        self.asin = re.search(r'/dp/(\w+)', url).group(1)
        # 平台
        self.platform = 1
        # 加购信息
        self.add_cart_info = ""
    ...
    def get_son_variant(self) -> list:
        ...
        try:
            ...
            if is_variant_one_str in variant_json['colorImages']:
                son_list = {k: v for k, v in self.data_to_return['dimensionValuesDisplayData'].items() if
                            k != self.data['asin']}
            elif ' '.join(is_variant[0]).replace('"', '\\"') in variant_json['colorImages']:
                son_list = {k: ' '.join(v).replace('"', '\\"') for k, v in
                            self.data_to_return['dimensionValuesDisplayData'].items() if k != self.data['asin']}
            else:
        ...
        for asin, types in son_list.items():
            ...
            if image_type in variant_json['colorImages'].keys():
                ...
                type_name = None
                logging.info(
                    f'子变体图片解析失败! asin:{asin} types:{types} colorImages:{variant_json["colorImages"].keys()}')
            try:
    ...
    def screen(self) -> int:
        ...
        if self.exception_type == 0:
            ...
            if not self.data['brand']:
                ...
                return 8
            # 筛选评论数
            if self.data['commentCount'] and self.data['commentCount'] < 1:
...
if __name__ == '__main__':
    _url = "https://www.amazon.com/dp/B004Y76374/ref=olp-opf-redir?aod=1&condition=new"
